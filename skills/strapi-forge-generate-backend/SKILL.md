---
name: strapi-forge-generate-backend
description: 'Generate complete Strapi v5 backend code including content-types, services, controllers, routes, and plugin lifecycle. Use when the user says "generate the plugin backend" or when invoked by the strapi-forge-create-plugin skill.'
---

# Generate Backend

**Goal:** Generate production-quality Strapi v5 server-side plugin code from an architecture blueprint — content-type schemas, components, services, controllers, routes, policies, middleware, plugin lifecycle, and TypeScript definitions. Typically invoked by the `strapi-forge-create-plugin` orchestrator with requirements and an architecture blueprint; when run standalone, gather those inputs from the user first.
