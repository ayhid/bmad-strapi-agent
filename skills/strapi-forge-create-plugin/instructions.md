...
<critical>This workflow orchestrates multiple specialist skills to generate a complete, production-quality Strapi v5 plugin</critical>
...
<action>Invoke the `strapi-forge-generate-backend` skill.

Pass to Rashid (strapi-forge-generate-backend skill):
...
<action>Invoke the `strapi-forge-generate-ui` skill.

Pass to Jamil (strapi-forge-generate-ui skill):
- Plugin requirements: {{plugin_requirements}}
- Architecture blueprint: {{plugin_architecture}}
...
<action>Invoke the `strapi-forge-generate-tests` skill.

Pass to Kamal (strapi-forge-generate-tests skill):
- Plugin requirements: {{plugin_requirements}}
- Architecture blueprint: {{plugin_architecture}}
...
<action>Invoke the `strapi-forge-review-plugin` skill.

Pass to Basir (strapi-forge-review-plugin skill):
...
<action>Invoke the `strapi-forge-generate-docs` skill.

Pass to Muin (strapi-forge-generate-docs skill):
- Plugin requirements: {{plugin_requirements}}
- Architecture blueprint: {{plugin_architecture}}
...
<action>Update plugin registry at {plugin_output_path}/plugin-registry.json:
- Add entry for this plugin
- Record creation date (ask or derive the current date)
...
<action if="option 2">Identify modifications needed and invoke the appropriate specialist skill (strapi-forge-generate-backend, strapi-forge-generate-ui, strapi-forge-generate-tests, strapi-forge-review-plugin, or strapi-forge-generate-docs)</action>
