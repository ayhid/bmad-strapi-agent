...
<critical>This skill is typically invoked by another Strapi Plugin Forge skill or agent - it expects a plugin code path as input. If any input is missing, ask the user for it before proceeding.</critical>
...
<action>Report review results to the invoking skill or agent:
...
Review report returned to the invoking skill or agent for next steps.
</action>

<action>Return review data to the invoking skill or agent:
